...
# if return true then it is out of bounds [log,high]
def out_of_bounds(position):
    for x in range(len(position)):
        if position[x] > high or position[x] < low:
            return True
    return False


def initialization():
    global c, ra, pc, pm, y, n, low, high, population, spiders
    ...
    n = 5  # dimensions
    # solution space or domain of definition [a , b]
    low = -10
    high = 10
...
def social_spider_algorithm():
    ...
    while number_of_iterations <= lim:
        ...
        for x in range(population):
            max_vibration = generate_vibration[x]
            ...
            if out_of_bounds(next_position):
                ...
                for t in range(n):
                    if next_position[t] > high:
                        next_position[t] = (high - spiders[x].s[t]) * rand_float
                    elif next_position[t] < low:
                        next_position[t] = (spiders[x].s[t] - low) * rand_float
